...
public class ReqUtils : System.Web.UI.Page
{
    public string GetHTML(string url, Encoding encoding)
    {
        ...
        | SecurityProtocolType.Ssl3;
            ...
            objRequest.UserAgent = @"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.106 Safari/537.36"; ;
            //objRequest.Proxy = wproxy;
    ...
    public ArrayList ExtractNewsLinks(string HtmlContent, string LinkPattern, string LinkDomainName)
    {
        ...
        while (m.Success)
        {
            try
            {
                ...
                if (LinkUrl != "" && LinkUrl.ToLower().Substring(0, 7) != "http://" && LinkUrl.ToLower().Substring(0, 8) != "https://")
    ...
    public string RemoveTags(string InputHtml, params string[] ExceptionTagList)
    {
            ...
            if (EndIndex > 0)
            {
                ...
                else
                {
                    for (int i = 0; i < ExceptionTagList.Length; i++)
                    {
                        ...
                        {
                            InnerContent = InnerContent.Replace("<" + TagContent + ">", "ALIREZASTART" + TagContent + "ALIREZAEND");
                        }
                    }
                }
                StartIndex = StartIndex - TagContent.Length;
        ...
        InnerContent = InnerContent.Replace("ALIREZASTART", "<");
        InnerContent = InnerContent.Replace("ALIREZAEND", ">");
